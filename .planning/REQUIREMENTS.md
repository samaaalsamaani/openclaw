...
- [x] **SERV-02**: All 6 launchd services start successfully on boot and stay running
...
- [x] **SERV-05**: File watcher monitors directories without missing events or dying
- [x] **SERV-06**: Heartbeat tasks (daily/weekly) execute successfully on schedule

### Integration Reliability (INTEG)

- [x] **INTEG-01**: Gateway ↔ Claude SDK calls succeed 99%+ (no ARG_MAX, timeout, or parsing errors)
...
- [x] **DATA-03**: Credential refresh works automatically (Late.dev tokens, OAuth renewal)
- [x] **DATA-04**: Codex OAuth token renewal process documented and automated (expires Mar 3!)
- [x] **DATA-05**: Remove placeholder 0-byte SQLite files (kb.sqlite, knowledge-base.sqlite, social.sqlite in ~/.openclaw/)
- [x] **DATA-06**: Config schema validation prevents invalid keys from crashing Gateway
- [x] **DATA-07**: Auth-profiles.json is single source of truth (no plist fallback causing drift)
...
- [x] **OBS-03**: Integration failure alerts (MCP timeouts, SDK errors logged to observability.sqlite)
- [x] **OBS-04**: Config corruption detection (validate before loading, alert on schema errors)
- [x] **OBS-05**: Credential expiry warnings (check all tokens, warn 7 days before expiry)
- [x] **OBS-06**: Daily health report (email/notification with status of all components)
...
| SERV-02     | Phase 16: Service Hardening              | Complete |
| SERV-03     | Phase 16: Service Hardening              | Complete |
| SERV-04     | Phase 16: Service Hardening              | Complete |
| SERV-05     | Phase 16: Service Hardening              | Complete |
| SERV-06     | Phase 16: Service Hardening              | Complete |
| INTEG-01    | Phase 17: Integration Reliability        | Complete |
...
| DATA-03     | Phase 18: Data Integrity & Config Safety | Complete |
| DATA-04     | Phase 18: Data Integrity & Config Safety | Complete |
...
| OBS-03      | Phase 19: Monitoring & Alerting          | Complete |
| OBS-04      | Phase 19: Monitoring & Alerting          | Complete |
| OBS-05      | Phase 19: Monitoring & Alerting          | Complete |
| OBS-06      | Phase 19: Monitoring & Alerting          | Complete |
