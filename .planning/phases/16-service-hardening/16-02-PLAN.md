...
Create src/infra/memory-monitor.test.ts validating:

- startMemoryMonitoring() returns cleanup function
- Memory history tracks last 12 samples (FIFO)
- Growth rate calculation is accurate (mock 12 samples with linear growth)
- No alert for growth <10MB/hour
- Alert logged for growth >10MB/hour
- Cleanup stops monitoring interval
...
<done>Memory monitoring module exists, tests pass, growth detection logic validated.</done>
</task>
...
Create src/infra/circuit-breaker.test.ts validating:

- Closed state allows calls through
- Opens after 5 consecutive failures
- Stays open for configured timeout
- Half-open tries one call after timeout
- Half-open → closed on success
- Half-open → open on failure
- Open state rejects immediately (no fn execution)
...
<done>Circuit breaker class exists, state transitions work correctly, tests validate all paths.</done>
</task>
