...
Create src/infra/crash-logger.test.ts validating:

- logServiceCrash() inserts row with correct fields
- Multiple crashes are logged independently
- SQLite connection errors don't throw (degradation)
- Test uses in-memory database (:memory:)
...
<done>Tests pass. logServiceCrash() function exists and writes to observability.sqlite events table.</done>
</task>
...
<done>server-close.ts imports crash-logger, tracks timers in Set, clears on shutdown, logs crashes on exit.</done>
</task>
...
<done>All MCP tool handlers wrapped in try/catch, return isError=true on failure, log errors without crashing.</done>
</task>
